classdef Controller < handle
    ...
    methods % controllers for connecting, disconnecting device, and closing app
    ...
    methods % controllers for trigger settings
        ...
        function controller_updateTriggerChannel(obj,src,~)
            ...
            if obj.viewObj.ChannelSettings.(TriggerChannelVal).ChannelEnable.Value == 0
                obj.viewObj.ChannelSettings.(TriggerChannelVal).ChannelEnable.Value = 1;
                obj.modelObj.callback_updateChannelEnableCheckBox(TriggerChannelVal,1);
            end
        end
    ...
    methods % controllers for channel settings
        function controller_updateChannelEnableCheckBox(obj,src,~)
            EnableVal = obj.ChannelEnableMaps(num2str(src.Value));
            ChannelName = src.Tag(end);
            obj.modelObj.callback_updateChannelEnableCheckBox(ChannelName,EnableVal); % This line
        end
        function controller_updateChannelCouplingSetting(obj,src,~)
            ChannelName = src.Tag(end);
            CouplingVal = obj.ChannelCouplingMaps(src.Value);
            obj.modelObj.callback_updateChannelCouplingSetting(ChannelName,CouplingVal);
        end
        function controller_updateChannelRangeSetting(obj,src,~)
            ChannelName = src.Tag(end);
            RangeVal = obj.ChannelRangeMaps(src.Value);
            obj.modelObj.callback_updateChannelRangeSetting(ChannelName,RangeVal);
        end
        function controller_updateChannelOffsetSetting(obj,src,~)
            ChannelName = src.Tag(end);
    ...
    methods % controllers for runing and stoping collection
    ...
    methods % controllers for saving, loading and clearing data
        ...
        function controller_clearAxes(obj,~,~)
            obj.modelObj.callback_clearAxesAndData();
        end
    end
